...
// missions[2]: 별 보러 옥상 가기, 우주/자연, isSpecial: true
...
describe('카드 컴포넌트', () => {
  describe('오늘의 카드 미션 목록', () => {
    test('초기 상태는 뒤집히지 않은 상태다', () => {
      const { container } = render(<TodayMissionCard mission={missions[0]} />);
    ...
    test('앞면 요소들이 렌더링된다 일반 미션의 경우', () => {
      render(<TodayMissionCard mission={missions[0]} />);
    ...
    test('앞면 요소들이 렌더링된다 히든 미션의 경우', () => {
      render(<TodayMissionCard mission={missions[2]} />);
  ...
  describe('카드가 뒤집힌 상태일 경우', () => {
    ...
    test('선택하기 클릭 시 도전자 수와 취소하기가 표시된다', async () => {
      ...
      expect(
        screen.getByText(
          `${missions[0].completedCount}명이 미션에 도전중이에요!`,
        ),
      ).toBeInTheDocument();
    ...
    test('넘기기 클릭 시 onSkip 콜백이 mission.id와 함께 호출된다', async () => {
      const user = userEvent.setup();
      const onSkip = jest.fn();
      const { container } = render(
        <TodayMissionCard mission={missions[0]} onSkip={onSkip} />,
      );

      await user.click(container.firstChild as HTMLElement);
      await user.click(screen.getByRole('button', { name: '넘기기' }));

      expect(onSkip).toHaveBeenCalledWith(missions[0].id);
    });

    test('선택하기 클릭 시 onSelect 콜백이 mission.id와 함께 호출된다', async () => {
      const user = userEvent.setup();
      const onSelect = jest.fn();
      const { container } = render(
        <TodayMissionCard mission={missions[0]} onSelect={onSelect} />,
      );
    ...
    test('취소하기 클릭 시 onCancel 콜백이 mission.id와 함께 호출된다', async () => {
      const user = userEvent.setup();
      const onCancel = jest.fn();
      const { container } = render(
        <TodayMissionCard mission={missions[0]} onCancel={onCancel} />,
      );
      ...
      expect(onCancel).toHaveBeenCalledWith(missions[0].id);
    });
  });

  describe('나의 미션 목록', () => {
    test('카드가 뒤집힌 상태로 렌더링된다', () => {
      render(<MyMissionCard mission={missions[2]} />);
    ...
    test('일반 미션의 경우 카테고리 이름이 나온다', () => {
      render(<MyMissionCard mission={missions[0]} />);
      expect(screen.getByText(missions[0].categoryName)).toBeInTheDocument();
    });
    ...
    test('히든 미션의 경우 카테고리 이름이 나오지 않고 히든 미션으로 나온다', () => {
      ...
      expect(screen.getByText('히든 미션')).toBeInTheDocument();
      expect(
        screen.queryByText(missions[2].categoryName),
      ).not.toBeInTheDocument();
    });
